...
해당 비즈니스 정보를 저장하지 못하거나 변경해야하는 상황이 발생할 수도 있다. <br/>
대리 키는 비즈니스와는 무관한 임의의 값으로 어떤 요구사항의 변화에도 계속 사용할 수 있는 장점이 있다. 그래서 테이블은 한 번 정의하면 쉽게 변경하기가 쉽지 않기때문에 어떤 변화도 수용할 수 있는 대리 키를 사용하는것이 권장된다.
...
@Column을 생략하면 기본값이 적용되는데, nullable은 예외가 있다. 자바의 기본타입은 not null을, 기본타입의 래퍼타입 객체는 null을 허용한다.  
예를 들어 int는 not null로 생성되지만, Integer는 null을 허용한다. 자바는 기본 타입에 null 값을 입력할 수 없기 때문인데 @Column을 생략하지 않는 경우에는
기본 타입은 nullable=false로 속성을 지정하는 것이 안전하다.
...
이 어노테이션은 따로 데이터베이스에 저장하지 않고 객체에서 임시로 어떤 값을 보관하고 싶을 때 사용한다.
